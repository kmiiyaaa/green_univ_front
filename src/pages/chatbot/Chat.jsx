...
export default function Chat({ variant = 'mono' }) {
	...
	// 안내 문구에 포함되는 [USER_CREATE], [BREAK_APP] 같은 토큰만 화면에서 숨김(기능에는 영향 없음)
	// 버튼/라우팅은 백엔드 links가 내려올 때만 그대로 렌더
	const stripBracketTokens = (text) => {
		if (!text) return text;

		// [ABC_DEF] 형태만 제거
		let out = String(text).replace(/\[[A-Z0-9_]+\]/g, '');

		// 토큰 지운 후 생기는 "여분 공백" 정리
		out = out.replace(/[ \t]{2,}/g, ' '); // 연속 공백
		out = out.replace(/\n{3,}/g, '\n\n'); // 연속 줄바꿈
		return out.trim();
	};
	...
	const send = async () => {
		...
		try {
			...
			// 안내문구에 포함된 [USER_CREATE] 같은 토큰만 화면에서 숨김
			const displayText = stripBracketTokens(answer + refText);
			pushBot(displayText);

			// 백엔드 links가 있으면 그대로 버튼 렌더(자동 생성은 제거)
