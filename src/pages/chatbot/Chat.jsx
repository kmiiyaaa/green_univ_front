...
export default function Chat({ variant = 'mono' }) {
  ...
  /* =========================
   * 4) 팝업 "확인" 클릭
   * - 지금은 localStorage 저장해두었지만, "항상 뜨게"면 저장해도 상관없음
   * - 한번만 뜨게를 쓰려면 아래 setItem이 핵심
   * ========================= */
  ...
  // 메세지 추가 헬퍼들
  const pushBot = (text) =>
    setMessages((p) => [...p, { id: uid(), role: 'bot', type: 'text', text }]);

  const pushLinks = (links) =>
    setMessages((p) => [...p, { id: uid(), role: 'bot', type: 'links', links }]);
